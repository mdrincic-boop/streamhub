...
import { Plus, Radio, Activity, Users, LogOut, User, Shield } from 'lucide-react';
...
function App() {
  ...
  const handleCreateStream = async (
    ...
    try {
      ...
      if (inputType === 'rtsp' && rtspConfig) {
        ...
        streamData.auto_reconnect = rtspConfig.autoReconnect;
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {liveStreams.map(stream => (
                    <StreamCard
                      ...
                      onSettings={setSettingsStream}
                      showSettings={user?.id === stream.created_by}
                    />
                  ))}
                </div>
              </div>
      ...
      {settingsStream && (
        <StreamSettingsModal
          streamId={settingsStream.id}
          onClose={() => setSettingsStream(null)}
          onSave={() => {
